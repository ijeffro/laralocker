...
class AttributeHandler extends APIHandler {
  ...
  public function url($id = null) {
    return trim($this->url . $this->api . $this->v2 . $this->persona_attribute, '/') . '/' . $id ?? $id;
  }
 ...
 public function get() {
  try {
    $response = $this->getFromLearningLocker(
  ...
  public function update($data) {
    try {
      $response = $this->patchToLearningLocker(
        $this->url($this->id ?? $this->id), $data
      );
      return $response;
    } catch (Exception $e) {
      return $e;
  ...
  public function create($data = null) {
    // dump($this->url(), $data); die;
    try {
      $response = $this->postToLearningLocker(
        $this->url(), $this->preparePayload($data)
      );
      return $response;
    } catch (Exception $e) {
      return $e;
  ...
  public function preparePayload($data) {
    ...
    if ($this->id && !array_key_exists('personaId', $data)) {
      $data['personaId'] = $this->id;
    }
